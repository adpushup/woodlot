...
var auth = require('basic-auth'),
    ...
    checkParam = require('./utils').checkParam;
...
function getLogParameters(req, res, startTime, config) {
    var params = { 
        ...
        method: checkParam(req.method), 
        url : checkParam(req.originalUrl), 
        ip: checkParam(req.ip), 
        body: checkParam(req.body), 
        params: checkParam(req.params), 
        query: checkParam(req.query), 
        httpVersion: checkParam(req.httpVersion), 
        statusCode: checkParam(res.statusCode), 
        timeStamp: generateCLFTimestamp(new Date()), 
        contentType: checkParam(res.get('Content-Type')), 
        contentLength: checkParam(res.get('Content-Length')),
...
function generateLog(req, res, startTime, config) {
    var params = getLogParameters(req, res, startTime, config),
        ...
        clfTimeStamp = generateCLFTimestamp(new Date()), // Generate CLF timestamp
