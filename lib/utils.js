...
var DEFAULT_JSON_SPACING = require('./constants').DEFAULT_JSON_SPACING,
    ...
    stdoutFormatting = require('./stdoutFormatting'),
...
function colorizeStatusCode(statusCode) {
    if (statusCode >= 200 && statusCode < 300) {
        ...
        return stdoutFormatting.foregroundGreen(statusCode);

    } else if (statusCode >= 300 && statusCode < 400) {

        return stdoutFormatting.foregroundBlue(statusCode);

    } else if (statusCode >= 400 && statusCode < 500) {

        return stdoutFormatting.foregroundYellow(statusCode);

    } else {

        return stdoutFormatting.foregroundRed(statusCode);
...
function colorizeLogLevel(level) {
    switch (level.toLowerCase()) {
        case 'debug':
            return stdoutFormatting.foregroundBlue(level);
        case 'warn':
            return stdoutFormatting.foregroundYellow(level);
        case 'err':
            return stdoutFormatting.foregroundRed(level);
        case 'info':
        default:
            return stdoutFormatting.foregroundGreen(level);
    }
};
    ...
    switch (format.toLowerCase()) {
        ...
        case 'customleveljson':
            var levelIndex = log.match(CUSTOMLEVELJSON_REGEX).index,
                colorizedLog = colorizeLogLevel(log.substr(levelIndex + 10, 5).trim().replace(/['"]+/g, ''));
...
module.exports = {
    ...
    logConfigWarning: logConfigWarning
};
