...
module.exports = {
	...
	colorizeStdoutFormat: function (log, format) {
		switch (format.toLowerCase()) {
			...
			case 'customleveljson':
				var levelIndex = log.match(constants.STDOUT_FORMAT_REGEX.CUSTOMLEVELJSON).index,
					colorizedLog = this.colorizeLogLevel(log.substr(levelIndex + 10, 5).trim().replace(/['"]+/g, ''));

				return log.replace(constants.STDOUT_FORMAT_REGEX.CUSTOMLEVELJSON, "level: " + '"' + colorizedLog + '"');

			case 'json':
			default:
				var statusCodeIndex = log.match(constants.STDOUT_FORMAT_REGEX.JSON).index + 14,
					statusCodeValue = log.substr(statusCodeIndex, 3),
					color = this.colorizeStatusCode(statusCodeValue);

				return log.replace(constants.STDOUT_FORMAT_REGEX.JSON, '"statusCode": ' + color);
		}
	}
};