...
function logToStdOut(log, format, spacing, config) {
    ...
    if (format === 'json') {
        ...
        // no need for colorizedLog if compact options is enabled
        if (spacing === null) {
            outputStream.write(stringifiedLog + utils.getFormatSeparator(config));
        } else {
            colorizedLog = stdoutFormatting.colorizeStdoutFormat(stringifiedLog, 'customLevelJSON');
            outputStream.write(colorizedLog + utils.getFormatSeparator(config));
        }
    } else {
        var colorizedLog = stdoutFormatting.colorizeStdoutFormat(log, 'text')
        outputStream.write(colorizedLog + utils.getFormatSeparator(config));
    }
};
