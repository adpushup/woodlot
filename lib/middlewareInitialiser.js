...
function logToStdOut(log, format, spacing, config) {
    ...
    if(format === 'json') {
        var colorizedLog = utils.colorizeStdoutFormat(utils.stringifyLog(log, format, spacing) + utils.getFormatSeparator(config), utils.getLogFormat(config));
        outputStream.write(colorizedLog);
    } else {
        outputStream.write(utils.colorizeStdoutFormat(utils.stringifyLog(log, format, spacing), utils.getLogFormat(config)));
    }
};
